...
router.get('/blog/:blogno/:title', async (req, res) => {
  ...
  let blog = await db.get().collection('blogs').findOne({ blogno:blogno })
  if (blog) {
...
router.get('/edit/:blogno/:title', async function (req, res) {
  let blogno = req.params.blogno
  blogno = Number(blogno)
  let user = await db.get().collection('users').findOne({ _id: ObjectId(req.session.user) })
  let blog = await db.get().collection('blogs').findOne({ blogno:blogno})
...
router.get('/delete/:blogno/:title',(req, res) => {
  let blogno = req.params.blogno
  blogno = Number(blogno)
  db.get().collection('blogs').deleteOne({ blogno:blogno })
  res.redirect('/users/')
})
